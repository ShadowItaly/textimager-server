    ...
    def run_component(self, component, or_parms, numagents, rmoverride, background, nodup, localdate):
        ...
        for c in complist:
            if ( c == 'agent' ):
                ...
                # memory limit start
                print "Loading memory limits file..."
                _memory_limits_filename = self.DUCC_HOME + "/resources/ducc.memory_limits"
                _memory_limit = None
                if os.path.exists(_memory_limits_filename):
                        _mfile = open(_memory_limits_filename)
                        for _ml in _mfile:
                                _ml = _ml.strip()
                                if not _ml:
                                        continue
                                ...
                                if len(_mlist) != 2:
                                        continue
                                if _mlist[0] == self.localhost:
                                        _memory_limit = str(int(_mlist[1])*1024*1024)
                                        break
                else:
                        print "Cannot read memory limits file", _memory_limits_filename
                print "Memory limit: ", _memory_limit
                if _memory_limit is not None:
                        print "Setting memory limit for this node..."
                        jvm_opts.append('-Dducc.agent.node.metrics.fake.memory.size=' + _memory_limit)
                # memory limit ende
