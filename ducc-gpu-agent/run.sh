...
if [ ! -e $DUCC_AGENT_INSTALLED ]; then
  ...
  echo "ducc agent gpu mapping: $TI_DOCKER_GPU_CONTAINER"

	# add this agent to head
	LOCAL_HOSTNAME=$(hostname)
	su - ducc -c "ssh ducc@$TI_DOCKER_HEAD_HOST \"bash /home/ducc/ducc/add-node.sh $LOCAL_HOSTNAME $TI_DOCKER_AGENT_MEMORY_LIMIT $TI_DOCKER_GPU_CONTAINER\""
